...
document.addEventListener('DOMContentLoaded', function() {
    // Create canvas and container elements
    ...
    // Custom cursor is not needed, we'll use the default cursor
    ...
    const densityFactor = 0.015; // Reduced density for better performance
    let damping = 0.97,
      stiffness = 0.12,
      iterations = 2;
    ...
    function resizeCanvas() {
      ...
      cw = canvas.width;
      ch = canvas.height;
      pointsX = Math.max(8, Math.floor(cw * densityFactor)); // Reduced for performance
      pointsY = Math.max(8, Math.floor(ch * densityFactor)); // Reduced for performance
      initMouse();
      initSimulation();
    }
    ...
    // Mouse event handlers (now applied to window instead of just canvas for full-page effect)
    ...
    document.addEventListener("mousedown", (e) => {
      ...
      if (selectedPoint === null) {
        // Only create ripple if not clicking an interactive element (links, buttons)
        if (e.target.tagName !== 'A' && e.target.tagName !== 'BUTTON' && 
            !e.target.closest('.social-icon') && !e.target.closest('.dropdown__button')) {
    ...
    function drawWeb() {
      // Clear the canvas with a dark fill.
      ctx.fillStyle = "rgba(17, 17, 17, 0.95)"; // Slightly transparent for content visibility
      ctx.fillRect(0, 0, cw, ch);
  
      // Add a radial gradient for fading effect at the edges
      const gradient = ctx.createRadialGradient(cw/2, ch/2, 10, cw/2, ch/2, ch);
      gradient.addColorStop(0, "rgba(34, 34, 34, 0)");
      gradient.addColorStop(0.8, "rgba(34, 34, 34, 0.5)");
      ...
      for (let y = 0; y < pointsY; y++) {
        ...
        let alpha = 0.3 + (y / pointsY) * 0.2; // Reduced opacity for background effect
      ...
      for (let x = 0; x < pointsX; x++) {
        ...
        ctx.strokeStyle = "rgba(50,50,50,0.3)"; // Reduced opacity
        ctx.lineWidth = 0.8;
        ctx.stroke();
      }
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        let brightness = 30; // Lower base brightness for background
        if (mouseOnCanvas && d < mouseInfluenceRadius) {
          brightness = Math.floor(
            30 + mouseEffectStrength * (1 - d / mouseInfluenceRadius) * 120
          );
        }
        ctx.fillStyle = `rgb(${brightness}, ${brightness}, ${brightness})`;
        ctx.beginPath();
        const radius = p.pinned ? 3 : 1.5; // Smaller dots for background
    ...
    function randomRipples() {
      if (Math.random() < 0.01 && !mouseDown) { // Reduced frequency for background
        createRipple(Math.random() * cw, Math.random() * ch);
      }
      setTimeout(randomRipples, 2000); // Increased interval
    }
